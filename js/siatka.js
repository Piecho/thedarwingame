function siatka () {
    ...
    function create() { //tworzenie obiektów
        ...
        textsiatka1 = game2.add.text(400, 560, "Aby przejść do głownej gry nacisnij Q.", { font:'30px Hobo, cursive',  fill: '#E85C2F' });
    ...
    function update() { //pętla główna  gry
        ...
        if (cursors2.A.isDown || cursors.left.isDown) {
        ...
        if (cursors2.W.isDown && game2.time.now > jumpTimer2 && checkIfCanJump2() || cursors.up.isDown && game2.time.now > jumpTimer2 && checkIfCanJump2())
        {
            player2.body.moveUp(400);
            jumpTimer2 = game2.time.now + 750;
        }

        if (cursors2.Q.isDown && glownaflag == true)
        {
            $('#siatka').remove();
            $('#glowna').show();
            glownaflag = false;
